...
function CharacterInPage() {
  ...
  useEffect(() => {
    axios
      .post(process.env.REACT_APP_API_URL + "getCharacters", params)
      .then((res) => {
        if (res.status === 200) {
  ...
  const handleAddNewCharacter = (item) => {
    alert("click on adding new character : " + item.uid);
  };

  const handleDeleteCharacter = (item) => {
    alert("click on delete character" + item.uid);
...
export default adminLayout(CharacterInPage);
